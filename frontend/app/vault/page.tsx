...
export default function VaultPage() {
  ...
  // ⭐ Fetch live pools
  const { pools: livePools, loading: poolsLoading } = usePools();

  // Convert backend pools → UI format
  const formattedPools: Pool[] = livePools?.map((p: any, index: number) => {
    const [token1, token2] = p.name?.split("/") || ["Unknown", "Unknown"];

    return {
      id: p.poolAddress || index.toString(),
      pair: p.name,
      token1: token1.trim(),
      token2: token2.trim(),
      tvl: p.tvl,
      volume24h: p.volume24h,
      utilization: Number(((p.volume24h / p.tvl) * 100).toFixed(2)),
      apr: p.apr,
    ...
    >
      ...
      <nav className="border-b-2 border-border bg-background/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-6 py-4">
          <div className="grid grid-cols-3 items-center">
            <div>
              <Link href="/" className="text-2xl font-bold">ZeroG</Link>
            </div>

            <div className="flex items-center justify-center gap-4">
              <Button variant="ghost" className="border-2 rounded-full px-6" asChild>
                <Link href="/dashboard">Dashboard</Link>
              </Button>
              <Button className="bg-foreground text-background border-2 rounded-full px-6" asChild>
                <Link href="/vault">Vault</Link>
              </Button>
              <Button variant="ghost" className="border-2 rounded-full px-6" asChild>
                <Link href="/swap">Swap</Link>
            ...
            <div className="flex items-center justify-end gap-4">
              {isConnected ? (
                <>
                  {petraWallet && (
                    <div className="hidden sm:flex items-center gap-2 px-4 py-2 border-2 bg-muted/50 rounded-full">
                      <span className="text-sm font-mono">{aptBalance} APT</span>
                    </div>
                  )}
                  <div className="flex items-center gap-2 px-4 py-2 border-2 bg-card rounded-full">
                    {photonToken && (
                ...
                <Button className="bg-foreground text-background rounded-full px-6 border-2" asChild>
                  <Link href="/signin">Sign In</Link>
                </Button>
              )}
            </div>
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <section className="mb-8">
          ...
          {poolsLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {formattedPools.slice(0, 3).map((pool) => (
                <VaultCard
                  key={pool.id}
                  vault={{
                    id: pool.id,
                  ...
                  onEnter={() => handleEnterVault(pool as any)}
                />
              ))}
            </div>
          )}
        ...
        {!isConnected && (
          <div className="text-center py-16">
            ...
            <Link href="/signin">
              <Button size="lg" className="bg-foreground text-background rounded-full">
                Connect Wallet
              </Button>
            </Link>
          </div>
